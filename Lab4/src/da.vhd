...
begin
  ...
  begin 
    -- TODO
    -- consider area efficient, use multiplexer instead of left-shift
    case count_p1_d is
      when "00" =>
        acc_sig <= resize(data_lut_p, 10);
      when "01" =>
        acc_sig <= acc_p2 + (resize(data_lut_p, 10)(8 downto 0) & '0');
      when "10" =>
        acc_sig <= acc_p2 + (resize(data_lut_p, 10)(7 downto 0) & "00");
      when "11" =>
        acc_sig <= acc_p2 - (resize(data_lut_p, 10)(6 downto 0) & "000");
      when others =>
        acc_sig <= (others => '0');
    end case;
  end process;
